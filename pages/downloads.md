...
2. `MINEUR` wordt verhoogd wanneer functionaliteit wordt toegevoegd _zonder_ de status van bestaande ORI-A XML te veranderen (ook wel "compatibele wijzigingen").
3. `PATCH` wordt verhoogd bij compatibele _bugfixes_, zoals correcties in de documentatie. Veranderingen van deze soort zijn beperkt tot aanpassingen binnen `<xs:annotation>` tags.
